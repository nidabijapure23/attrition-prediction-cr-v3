...
def process_predictions(df):
    ...
    cost_center_encoder = TargetEncoder(cols=['Cost Center'], smoothing=1)
    cost_center_encoder.fit(X[['Cost Center']], y)
...
def save_false_positives_to_sheet(false_positives_df):
    """
    Save false positive predictions to Google Sheet.
    """
    try:
    ...
    if st.button("Predict"):
        with st.spinner('Running prediction...'):
            ...
            if not false_positives.empty:
                ...
                if save_false_positives_to_sheet(false_positives):
                    st.success("Predictions have been saved to the tracking sheet.")
                else:
                    st.warning("Failed to save false positive predictions to tracking sheet.")
            else:
